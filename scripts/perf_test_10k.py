#!/usr/bin/env python3
"""
Performance test script for MRP system - Smaller dataset (10,000 materials)
"""
import subprocess
import time
import json
...
def run_sql(sql, database="mrp"):
    """Execute SQL via Docker using stdin pipe"""
    cmd = [
...
def generate_test_data():
    """Generate forecast materials and insert into database"""
    ...
    for batch_start in range(0, MATERIAL_COUNT, BATCH_SIZE):
        ...
        print(f"\rProgress: {progress:.1f}% ({batch_end}/{MATERIAL_COUNT}) - "
              f"{total_rows} rows - {elapsed:.1f}s", end='', flush=True)
...
def verify_results(plan_version_id):
    ...
    # Verify batch write size
    result = run_sql(f"""
        SELECT 
            MIN(cnt) as min_batch,
            MAX(cnt) as max_batch,
            AVG(cnt) as avg_batch
        FROM (
            SELECT COUNT(*) as cnt 
            FROM plan_detail 
    ...
    """)
    print(f"Batch write stats (rows per week):")
...
def main():
    print("=" * 60)
    print("MRP Performance Test: 10,000 Materials × 12 Weeks")
    print("=" * 60)
